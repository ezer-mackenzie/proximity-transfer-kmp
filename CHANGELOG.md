...
- First major stable release of Proximity Transfer Kotlin Multiplatform library.
- Comprehensive end-to-end integration test suite (`ProximityTransferV1IntegrationTest`).

## [0.9.0] - 2026-08-31

### Added

- Session event lifecycle hierarchy and listeners (`TransferSessionEvent`, `TransferSessionListener`, `DefaultTransferSessionListener`).
- Fluent client engine builder for custom engine configuration (`ProximityTransferClientBuilder`).
...
- Security handshake key exchange engine for dynamic session secret key derivation (`SessionKeyExchangeEngine`).
- Real-time transfer metrics and progress calculator (`TransferProgressMetrics`, `TransferProgressCalculator`).
